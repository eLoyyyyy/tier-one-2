...
class wp_materialize_navwalker extends Walker {
    ...
    function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        $attributes .= ! empty( $children )         ? ' data-activates="dropdown-'. $item->ID .'" data-beloworigin="true" ' : '';
...
function create_materialize_submenu( $theme_location ) {
    if ( ($theme_location) && ($locations = get_nav_menu_locations()) && isset($locations[$theme_location]) ) {
        ...
        if ( !empty($logo) ) : 
            $menu_list .= '<li>';
                $menu_list .= '<div class="userView" style="height:160px;">';
                    $menu_list .= '<div class="background" >';
